...
SINGLE_STAGE_TASK_DATASETS = OrderedDict(
    ( _task_name_from_file(fp), dict(horizon=500, human_path=fp) )
    for fp in libero_files
)
...
def get_new_ds_path(task, ds_type, return_info=False):
    ...
    ds_path = ds_config["human_path"]  # already the full .hdf5 path
...
class VideoDataset(Dataset):
    def __init__(
        ...
        #self.task_list = list(tasks.keys()) if (tasks is not None and len(tasks) > 0) else list(SINGLE_STAGE_TASK_DATASETS.keys())
        ...
        for task_index, task_name in enumerate(self.task_list):
            ...
            if self.mode == "test":
                for demo_key in task_data.keys():
                    ...
                    task_description_with_steps_tokenized = open_clip.tokenize(task_description)  #open_clip.tokenize(steps) # returns torch.Size([1, 77])
                ...
                for demo_key in task_data.keys():
                    ...
                    # if steps != None:
                    task_description_with_steps_tokenized =  open_clip.tokenize(task_description)  #open_clip.tokenize(steps)
                    # else:
    ...
    def __getitem__(self, i):
        ...
        try:
            ...
            # keep your 3-stream format by duplicating agentview as left/right
            left_image = agentview
            right_image = agentview
            gripper_image = eye_in_hand
