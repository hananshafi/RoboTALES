...
class CycleReward(nn.Module):
    ...
    def score(self, image, prompt):
        device_new = self.blip.text_encoder.device
        text_input = self.blip.tokenizer(prompt, padding='max_length', truncation=True, max_length=self.max_length, return_tensors="pt").to(device_new)  #to(self.device)
