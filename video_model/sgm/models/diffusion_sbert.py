...
class DiffusionEngine(pl.LightningModule):
    ...
    def get_sbert_loss(
        ...
        """
        Returns ONLY the DDPO term (caller can do: total = supervised + returned_ddpo).

        Key behavior:
        - video and pose can have DIFFERENT T (T_video != T_pose)
        - rollout uses a *true* Euler-Ancestral step (sigma_down/sigma_up) when eta>0
        - DDPO logp matches the *exact same* kernel used in rollout
        - reward: BLIP caption -> BERTScore (Recall) vs orig_task_string prompt
        - per-prompt running mean/std baseline (advantages), then clip only
        """
        import math
        import torch
        import torch.nn.functional as F
        ...
        def euler_ancestral_step_fixed(x_t, sigma_t_bd, sigma_next_bd, x0_pred, noise, eta: float):
            """
            Consistent Euler-Ancestral step for decreasing sigmas (EDM-style):
        ...
        def extract_prompt_keys_for_baseline(add_inputs: Dict[str, Any], B: int, BxT_v: int, T_v: int) -> list[str]:
            """
            Prefer baseline key from 'orig_task_string' (what reward uses).
            Fall back to _extract_prompts if absent.
            """
            ...
            if isinstance(p, (list, tuple)):
                ...
                if len(p2) < B:
                    p2 = p2 + [p2[-1]] * (B - len(p2))
                return [str(p2[i]) for i in range(B)]
            else:
                return [str(p)] * B
        ...
        if BxT_v % T_v_decl != 0:
            raise RuntimeError(
                f"[DDPO] Packed video len {BxT_v} not divisible by declared T_video={T_v_decl}. "
                f"Fix dataloader or set batch['num_video_frames'] correctly."
            )
        T_v = T_v_decl
        B = BxT_v // T_v

        # reconcile V/Fv indexing if needed
        if V * Fv > T_v:
            Fv_eff = max(1, T_v // V)
        ...
        # ------------------- reward (BLIP -> BERTScore Recall) -------------------
        rm_device = self._blip_move_to(device)
        ...
        with torch.inference_mode():
            for s in range(0, len(decode_indices), decode_mb):
                ...
                # Always build PIL batch (avoid _HAS_PIL / undefined pil_batch issues)
                ...
                blip_inputs = self.blip_processor(images=pil_batch, return_tensors="pt").to(rm_device)
                generated_ids = self.blip_model.generate(**blip_inputs, max_new_tokens=30, num_beams=3)
                preds = self.blip_processor.batch_decode(generated_ids, skip_special_tokens=True)
                ...
                sc = R.to(device=device, dtype=torch.float32).view(-1)  # paper-aligned
                ...
                del z, img, img_u8_cpu, pil_batch, blip_inputs, generated_ids, preds, caps, sc, b_chunk, v_chunk, flat_idx
        ...
        for rec in traj:
            ...
            while b0 < B:
                ...
                sig1d_t_v, sig_t_v_bd = mk_sigmas(s_t, m_v, xt.ndim, device, xt.dtype)  # sig_t_v_bd: (m,1,1,1)
                ...
                # --- logp matches euler_ancestral_step_fixed ---
                xt32 = xt.float()
                xtm1_32 = xtm1.float()
                ...
                if eta == 0.0:
                    # deterministic transition -> no valid Gaussian logp; skip contribution
                    ...
                    sigma_up_safe = sigma_up.clamp_min(1e-6)  # if you clamp here, clamp in rollout too
                    resid = (xtm1_32 - x_mean32) / sigma_up_safe
                    resid = resid.clamp_(-10.0, 10.0)

                    resid2_mean = resid.view(m_v, -1).pow(2).mean(dim=1)                 # (m_v,)
                    log_sigma = torch.log(sigma_up_safe.reshape(m_v) + 1e-12)            # (m_v,)
                    logp_flat = (-0.5 * resid2_mean - log_sigma).to(loss_value.dtype)   # (m_v,)
    ...
    def forward(self, x, batch):
        ...
        sbert_reward = self.ddpo_alpha * self.get_sbert_loss(loss_mean, self.model, self.denoiser, self.conditioner, x, batch, sigmas, noised_input, model_output)
        loss_total = loss_mean  + sbert_reward
        loss_dict = {"loss": loss_total, "loss_diffusion": loss_mean,"loss_ddpo": sbert_reward}
