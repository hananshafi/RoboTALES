...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "single checkpoint file to resume from. "
            "For Lightning>=2, this is forwarded to Trainer.fit(ckpt_path=...)."
        ),
    )
...
def get_checkpoint_name(logdir):
    ...
    print(ckpt)
    if len(ckpt) == 0:
        # Some runs are configured with save_last=False and only have epoch/step checkpoints.
        fallback = natsorted(glob.glob(os.path.join(logdir, "checkpoints", "*.ckpt")))
        fallback = [p for p in fallback if "trainstep_checkpoints" not in p]
        if len(fallback) == 0:
            raise FileNotFoundError(
                f"No checkpoints found in {os.path.join(logdir, 'checkpoints')}. "
                "Use --resume_from_checkpoint /path/to/epoch=...ckpt for direct resume."
            )
        ckpt = sorted(fallback, key=lambda x: os.path.getmtime(x))[-1]
        print(f'No "last*.ckpt" found, using most recent checkpoint: {ckpt}')
        version = 1
...
if __name__ == "__main__":
    ...
    try:
        ...
        if ckpt_resume_path is not None:
            print(f"Trainer will resume from: {ckpt_resume_path}")
        ...
        sparse_ddpo = bool(getattr(model, "use_ddpo", False)) and int(
            getattr(model, "ddpo_every", 1)
        ) > 1
        ...
        if sparse_ddpo:
            # DDPO every-N-steps changes which params are active across iterations.
            # This is incompatible with static graph.
            default_strategy_params["find_unused_parameters"] = True
            default_strategy_params["static_graph"] = False
            print(
                "[DDP] Detected sparse DDPO schedule; forcing "
                "find_unused_parameters=True and static_graph=False."
            )
        default_strategy_config["params"] = default_strategy_params
        ...
        strategy_target = str(strategy_cfg.get("target", ""))
        if sparse_ddpo and "DDPStrategy" in strategy_target:
            if "params" not in strategy_cfg or strategy_cfg["params"] is None:
                strategy_cfg["params"] = OmegaConf.create()
            strategy_cfg["params"]["find_unused_parameters"] = True
            strategy_cfg["params"]["static_graph"] = False
        print(
            f"strategy config: \n ++++++++++++++ \n {strategy_cfg} \n ++++++++++++++ "
        )
        ...
        if not cpu:
            devices_cfg = lightning_config.trainer.devices
            if isinstance(devices_cfg, (list, tuple, ListConfig)):
                ngpu = len(devices_cfg)
            elif isinstance(devices_cfg, int):
                ngpu = int(devices_cfg)
            elif isinstance(devices_cfg, str):
                cleaned = devices_cfg.strip().strip(",")
                if cleaned == "":
                    ngpu = 1
                elif "," in cleaned:
                    ngpu = len([x for x in cleaned.split(",") if x.strip() != ""])
                else:
                    # Single id like "0" or single-count form.
                    ngpu = 1
            else:
                ngpu = 1
        else:
        ...
        if MULTINODE_HACKS:
            ...
            resp = requests.get("http://169.254.169.254/latest/meta-data/instance-id", timeout=1)
            print(
