...
class DiffusionEngine(pl.LightningModule):
    def __init__(
        ...
        vcd_enabled: bool = False,
        vcd_weight: float = 0.0,
        vcd_type: str = "simple"
    ):
        super().__init__()
        ...
        self.vcd_enabled = bool(vcd_enabled)
        self.vcd_weight = float(vcd_weight)
        self.vcd_type = str(vcd_type).lower()

        if self.vcd_enabled:
            if self.vcd_type == "simple":
                from vcd_supervised_simple import SimpleVCDLoss
                self.vcd_loss_fn = SimpleVCDLoss(loss_type="l1")
            else:  # "full"
                from vcd_supervised_full import FullVCDSupervisedLoss
                self.vcd_loss_fn = FullVCDSupervisedLoss(device='cpu')
    ...
    def _resolve_num_video_frames(self, batch: Dict, total_frames: int) -> int:
        """Extract num_video_frames from batch with validation."""
        num_video_frames = batch.get("num_video_frames", getattr(self, "num_video_frames", 0))
        
        if torch.is_tensor(num_video_frames):
            if num_video_frames.numel() == 0:
                return 0
            num_video_frames = int(num_video_frames.reshape(-1)[0].item())
        else:
            num_video_frames = int(num_video_frames)
        
        if num_video_frames <= 1:
            return 0
        if total_frames % num_video_frames != 0:
            return 0
    ...
    def forward(self, x, batch):
        # 'x' currently contains raw pixels [256, 256]
        # You MUST convert them to latents [32, 32]
    
        # 1. Encode pixels to latents
        with torch.no_grad():
            x["video"] = self.encode_first_stage(x["video"]).detach()
            # Now x["video"] will be torch.Size([100, 4, 32, 32])
        ...
        # 1) VCD supervised loss 
        vcd_loss = loss_mean.new_zeros(())
        phase = getattr(self.optimizer_config, "phase", "stage_1")
        
        if self.vcd_enabled and self.vcd_weight > 0.0 and phase == "stage_1":
            num_frames = self._resolve_num_video_frames(batch, x["video"].shape[0])
            
            if num_frames >= 2:
                if self.vcd_type == "simple":
                    vcd_loss = self.vcd_loss_fn.compute_loss(
                        pred_video=model_output,
                        target_video=x["video"],
                        num_video_frames=num_frames
                    )
                else:  # "full"
                    vcd_loss = self.vcd_loss_fn.compute_loss(
                        pred_latents=model_output,
                        target_latents=x["video"],
                        num_video_frames=num_frames,
                        decode_fn=self.decode_first_stage,
                        scale_factor=self.scale_factor
                    )
        
        # 2) DDPO loss
        ddpo_loss = loss_mean.new_zeros(())
        if self.use_ddpo and phase == "stage_1":
            ddpo_loss = self.ddpo_alpha * self.get_cyclereward_loss_modified(
                loss_mean, self.model, self.denoiser, self.conditioner,
                x, batch, sigmas, noised_input, model_output
            )
        
        # 3) Combine ALL THREE
        #loss_total = loss_mean
        loss_total = loss_mean + (self.vcd_weight * vcd_loss) + ddpo_loss
        
        # Logging
        loss_dict = {
            "loss": loss_total,
            "loss_diffusion": loss_mean,
        }
        if self.vcd_enabled and vcd_loss.item() > 0:
            loss_dict["loss_vcd"] = vcd_loss
        if self.use_ddpo and ddpo_loss.item() > 0:
            loss_dict["loss_ddpo"] = ddpo_loss
        
        return loss_total, loss_dict

    def shared_step(self, batch):
        # 1. Prepare 'x' (the video/pose tensors)
        x = self.get_input(batch)
        
        # 2. Pass both to forward
