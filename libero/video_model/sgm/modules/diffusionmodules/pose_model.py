...
class VideoResNet(nn.Module):
    def __init__(
        ...
        self.layer8 = self._make_layer(block, conv_maker, 1152, 960, emb_planes, blocks=2, temporal_stride=1, stride=1, num_groups=num_groups)
        self.layer9 = self._make_layer(block, conv_maker, 960, 960, emb_planes, blocks=2, temporal_stride=1, stride=1, num_groups=8)
        ...
        self.layer17 = self._make_layer(BottleneckExp2, conv_maker, 420, 420, emb_planes, blocks=4, temporal_stride=3, stride=1, num_groups=7)
    ...
    def forward(self, inputs: List[th.Tensor], emb: th.Tensor, num_pose_frames) -> th.Tensor:
        ...
        x1 = self.layer1_1(inputs[0]['gopro'], emb)
        x_cat = self.layer2_1_cat(inputs[1]['gopro'], emb)
        x1 = torch.cat([x1, x_cat], dim=1)
        x1 = self.layer2_1(x1, emb)
        x_cat = self.layer3_1_cat(inputs[2]['gopro'], emb)
        x1 = torch.cat([x1, x_cat], dim=1)
        x1 = self.layer3_1(x1, emb)
        x_cat = self.layer4_1_cat(inputs[3]['gopro'], emb)
        x1 = torch.cat([x1, x_cat], dim=1)
        x1 = self.layer4_1(x1, emb)
        x_cat = self.layer5_1_cat(inputs[4]['gopro'], emb)
        x1 = torch.cat([x1, x_cat], dim=1)
        x1 = self.layer5_1(x1, emb)
        x1 = self.layer6_1(x1, emb)
        x1 = self.layer7_1(x1, emb)

        x2 = self.layer1_2(inputs[0]['depth'], emb)
        x_cat = self.layer2_2_cat(inputs[1]['depth'], emb)
        x2 = torch.cat([x2, x_cat], dim=1)
        x2 = self.layer2_2(x2, emb)
        x_cat = self.layer3_2_cat(inputs[2]['depth'], emb)
        x2 = torch.cat([x2, x_cat], dim=1)
        x2 = self.layer3_2(x2, emb)
        x_cat = self.layer4_2_cat(inputs[3]['depth'], emb)
        x2 = torch.cat([x2, x_cat], dim=1)
        x2 = self.layer4_2(x2, emb)
        x_cat = self.layer5_2_cat(inputs[4]['depth'], emb)
        x2 = torch.cat([x2, x_cat], dim=1)
