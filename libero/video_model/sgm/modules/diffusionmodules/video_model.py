...
class VideoUNet(nn.Module):
    ...
    def forward(
        ...
        if self.pose_net_params is not None:
            ...
            transformed_inputs = [
                {
                    "gopro": rearrange(x, "(b t) c h w -> b c t h w", t=25).contiguous()[:, :, 1:, :, :].contiguous()[:, :, 0:(1*24//2), :, :].contiguous(),
                    "depth": rearrange(x, "(b t) c h w -> b c t h w", t=25).contiguous()[:, :, 1:, :, :].contiguous()[:, :, (1*24//2):, :, :].contiguous(),
                }
