            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    ...
                    if embedder.input_key == 'cond_frames':
                        ...
                        repeat_n = 12               # how many copies of each extra embedding
                        pieces = []
...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def forward(self, image, no_dropout=False): 
        # print(f"image: {image.shape}") # if caption size is [1, 77] with batch of 2, image.shape is [2, 77]
        # z = self.encode_with_vision_transformer(image)  # image shape torch.Size([14*b, 3, 256, 448]), z shape torch.Size([14*b, 1024])
        ...
        # print(f"xt: {xt.shape}")
        # print(f"xt datatype: {xt.dtype}")
        # print(f"x: {x.shape}")
        # print(f"x datatype: {x.dtype}")
        # exit()
        # import torch.nn.functional as F
        
        # cosine_sim = F.cosine_similarity(z, xt, dim=1)
    ...
    def encode_text(self, text):
        ...
        # print(f"x:{x.shape}")
        # exit()
        # print(f"inside x datatype: {x.dtype}")
